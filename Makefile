...
test-supports-php:
	...
	@docker-compose -f docker-compose.yml run --rm php sh -c "rm -f composer.lock && rm -rf vendor && composer install -q -n --no-progress && vendor/bin/pest --compact"
	...
	@docker-compose -f docker-compose.yml run --rm php sh -c "rm -f composer.lock && rm -rf vendor && composer install -q -n --no-progress"
